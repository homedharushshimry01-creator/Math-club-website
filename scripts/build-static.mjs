...
async function main() {
  ...
  const esbuild = isWindows
    ? path.join(root, "node_modules", ".bin", "esbuild.cmd")
    : path.join(root, "node_modules", ".bin", "esbuild");

  if (isWindows) {
    execFileSync("cmd.exe", ["/c", esbuild,
      path.join(root, "src", "static-main.tsx"),
      "--bundle", "--format=esm", "--platform=browser",
      "--target=es2020", "--loader:.png=dataurl",
      `--outfile=${path.join(dist, "app.js")}`, "--minify",
    ], { stdio: "inherit" });
  } else {
  ...
  const scanner = new Scanner({
    sources: [{ base: root, pattern: "src/**/*.{ts,tsx,js,jsx,html}", negated: false }],
  });
  ...
  const optimizedCss = optimize(generatedCss, { file: "style.css", minify: true }).code;
